...
#[no_mangle]
pub extern "C" fn wcg_core_ccow_destroy(cow: *mut CCow) {
    if cow.is_null() {
        return;
    }
...
#[no_mangle]
pub extern "C" fn wcg_core_ccow_get_bytes(cow: *const CCow) -> *const u8 {
    if cow.is_null() {
        return std::ptr::null();
    }
    let cow = unsafe { &*cow };
...
#[no_mangle]
pub extern "C" fn wcg_core_ccow_get_length(cow: *const CCow) -> usize {
    if cow.is_null() {
        return 0;
    }
    let cow = unsafe { &*cow };
